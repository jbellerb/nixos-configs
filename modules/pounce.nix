  ...
  settingsFormat = {
    ...
    generate =
      ...
      let
        ...
        mkKeyValueList = values: lib.generators.toKeyValue { inherit mkKeyValue; } values;
      in
      pkgs.writeText name (mkKeyValueList value);
  };
...
in
{
  options.services.pounce = {
    enable = mkEnableOption (lib.mdDoc "the Pounce IRC bouncer and Calico dispatcher");
        ...
        types.submodule {
          options = {
            ...
            notify = mkOption {
              ...
              default = { };
              example = {
                script = "/var/lib/pounce/notify.sh";
              };
              description = lib.mdDoc ''
              ...
              serviceConfig = {
                ...
                ExecStart = ''
                  ${pkgs.pounce-extra}/bin/pounce-notify \
                     ${
                       if value.notify.insecure then
                         "-!"
                       else if value.notify.trust-cert == "" then
                         "-t ${cfg.certDir}/${name}.${cfg.host}/fullchain.pem"
                       else if value.notify.trust-cert != null then
                         "-t ${value.notify.trust-cert}"
                       else
                         ""
                     } \
                     ...
                     ${
                       if cfg.networks.${name}.config ? local-pass then
                         "-w ${cfg.networks.${name}.config.local-pass}"
                       else
                         ""
                     } \
                     ${name}.${cfg.host} \
                     ${
                       if value.notify.command != "" then
                         "\"${value.notify.command}\""
                       else
